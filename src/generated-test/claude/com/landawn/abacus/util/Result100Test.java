...
public class Result100Test extends TestBase {
    ...
    public void testOrElseThrowWithSupplier() {
        Result<String, Exception> success = Result.of("value", null);
        Assertions.assertEquals("value", success.orElseThrow(() -> new RuntimeException("should not be thrown")));

        Result<String, Exception> failure = Result.of(null, new Exception("original"));

        RuntimeException thrown = Assertions.assertThrows(RuntimeException.class, () -> failure.orElseThrow(() -> new RuntimeException("custom error")));
