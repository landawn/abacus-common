 ...
 * containing five elements of potentially different types.
 *
...
public class Tuple5Type<T1, T2, T3, T4, T5> extends AbstractType<Tuple5<T1, T2, T3, T4, T5>> {
     ...
     *
     * @return {@code Tuple5.class}
     */
    @Override
    public Class<Tuple5<T1, T2, T3, T4, T5>> javaType() {
        return typeClass;
    }

    /**
     * Returns the parameter types of this tuple type.
     * The returned array contains the types of all five elements in order.
     *
     ...
     * Tuple5Type is always a generic type as it has type parameters.
     *
     * @return {@code true} always, as Tuple5 is a parameterized type
     */
    @Override
    ...
    public Tuple5<T1, T2, T3, T4, T5> valueOf(final String str) {
        ...
        if (a == null || a.length < 5) {
            throw new IllegalArgumentException("Invalid Tuple5 format. Expected array with at least 5 elements but got: " + str);
        }
    ...
    /**
     * Appends the string representation of the Tuple5 to the given Appendable.
     * The output format is: [element1, element2, element3, element4, element5]
     * Special handling is provided for Writer instances to improve performance.
     *
     * @param appendable the Appendable to write to
     * @param x the Tuple5 object to append
    ...
    /**
     * Writes the character representation of the Tuple5 to the given CharacterWriter.
     * The output format is: [element1, element2, element3, element4, element5]
     * This method is optimized for character-based output streams.
     *
     * @param writer the CharacterWriter to write to
     * @param x the Tuple5 object to write
     * @param config the serialization configuration
     * @throws IOException if an I/O error occurs during the write operation
     */
    @Override
    public void writeCharacter(final CharacterWriter writer, final Tuple5<T1, T2, T3, T4, T5> x, final JsonXmlSerConfig<?> config) throws IOException {
        if (x == null) {
    ...
    protected static String getTypeName(final String t1TypeName, final String t2TypeName, final String t3TypeName, final String t4TypeName,
        ...
        if (isDeclaringName) {
            return ClassUtil.getSimpleClassName(Tuple5.class) + SK.LESS_THAN + TypeFactory.getType(t1TypeName).declaringName() + SK.COMMA_SPACE
                    + TypeFactory.getType(t2TypeName).declaringName() + SK.COMMA_SPACE + TypeFactory.getType(t3TypeName).declaringName() + SK.COMMA_SPACE
                    + TypeFactory.getType(t4TypeName).declaringName() + SK.COMMA_SPACE + TypeFactory.getType(t5TypeName).declaringName() + SK.GREATER_THAN;
        } else {
            return ClassUtil.getCanonicalClassName(Tuple5.class) + SK.LESS_THAN + TypeFactory.getType(t1TypeName).name() + SK.COMMA_SPACE
                    + TypeFactory.getType(t2TypeName).name() + SK.COMMA_SPACE + TypeFactory.getType(t3TypeName).name() + SK.COMMA_SPACE
                    + TypeFactory.getType(t4TypeName).name() + SK.COMMA_SPACE + TypeFactory.getType(t5TypeName).name() + SK.GREATER_THAN;
        }
    }
}
