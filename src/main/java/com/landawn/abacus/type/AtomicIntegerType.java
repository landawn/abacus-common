...
public class AtomicIntegerType extends AbstractAtomicType<AtomicInteger> {
    ...
    /**
     * Parses an integer string and returns a new {@link java.util.concurrent.atomic.AtomicInteger}
     * containing the parsed value. Leading and trailing whitespace is trimmed before parsing; the trimmed text is
     * then parsed with the same grammar as {@link Numbers#toInt(String)}; decimal is tried first (a leading zero is padding, never octal), a
     * {@code 0x}/{@code 0X}/{@code #} prefix selects hexadecimal, a trailing {@code L}/{@code l} long-literal suffix is
     * ignored, and only ASCII digits are accepted. This is a different grammar from {@link Integer#parseInt(String)}:
     * non-ASCII Unicode digits such as Arabic-Indic digits are rejected where {@code parseInt} accepts them, and
     * {@code "0x10"}/{@code "#10"}/{@code "1L"} are accepted where {@code parseInt} rejects them.
     *
     ...
     */
    @Override
    public AtomicInteger valueOf(final String str) throws NumberFormatException, ArithmeticException {
        ...
        // Numbers.toInt("") answers 0; a blank string has always been rejected by this handler and must stay rejected.
        if (trimmed.isEmpty()) {
            throw new NumberFormatException("Blank string is not a valid int: \"" + str + "\"");
        }

        return new AtomicInteger(Numbers.toInt(trimmed));
    }
     ...
     */
    @Override
    public AtomicInteger get(final ResultSet rs, final int columnIndex) throws NullPointerException, SQLException {
     ...
     */
    @Override
    public AtomicInteger get(final ResultSet rs, final String columnName) throws NullPointerException, SQLException {
     ...
     */
    @Override
    public void set(final PreparedStatement stmt, final int columnIndex, final AtomicInteger x) throws NullPointerException, SQLException {
        if (x == null) {
     ...
     */
    @Override
    public void set(final CallableStatement stmt, final String parameterName, final AtomicInteger x) throws NullPointerException, SQLException {
        if (x == null) {
     ...
     */
    @Override
    public void appendTo(final Appendable appendable, final AtomicInteger x) throws NullPointerException, IOException {
        if (x == null) {
     ...
     */
    @Override
    public void serializeTo(final CharacterWriter writer, final AtomicInteger x, final JsonXmlSerConfig<?> config) throws NullPointerException, IOException {
        if (x == null) {
