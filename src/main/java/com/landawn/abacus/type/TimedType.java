...
public class TimedType<T> extends AbstractType<Timed<T>> {
    ...
    private final Class<Timed<T>> typeClass = (Class) Timed.class;
     ...
     */
    @Override
    public void writeCharacter(CharacterWriter writer, Timed<T> x, SerializationConfig<?> config) throws IOException {
        if (x == null) {
            writer.write(NULL_CHAR_ARRAY);
        } else {
            try {
