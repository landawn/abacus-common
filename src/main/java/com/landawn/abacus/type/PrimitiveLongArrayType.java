...
public final class PrimitiveLongArrayType extends AbstractPrimitiveArrayType<long[]> {
     ...
     * // str equals "[1, 2, 3]"
     *
     * String empty = type.stringOf(new long[0]);
     * // empty equals "[]"
     *
     ...
     * @throws ClassCastException if an element is not a {@code Long}.
    ...
    @MayReturnNull
    @Override
    public long[] collectionToArray(final Collection<?> c) throws ClassCastException, NullPointerException, ArrayIndexOutOfBoundsException {
        if (c == null) {
            return null; // NOSONAR
        }
