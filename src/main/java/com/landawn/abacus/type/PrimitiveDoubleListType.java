...
public final class PrimitiveDoubleListType extends AbstractPrimitiveListType<DoubleList> {
    ...
    public DoubleList valueOf(String str) {
        return N.isNullOrEmpty(str) ? null : DoubleList.of(arrayType.valueOf(str));
    }
     ...
     */
    @Override
    public void write(Writer writer, DoubleList x) throws IOException {
        if (x == null) {
     ...
     */
    @Override
    public void writeCharacter(CharacterWriter writer, DoubleList x, SerializationConfig<?> config) throws IOException {
        if (x == null) {
            writer.write(NULL_CHAR_ARRAY);
        } else {
