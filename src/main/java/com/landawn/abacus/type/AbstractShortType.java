...
public abstract class AbstractShortType extends NumberType<Number> {
     ...
     */
    @Override
    public Short valueOf(final String str) throws NumberFormatException, ArithmeticException {
     ...
     * {@code 0x}/{@code 0X}/{@code #} prefix) and checks that the result is within {@code short} range
     ...
     * @throws ArithmeticException if the region is a well-formed integer outside the {@code short} range
     */
    @Override
    public Short valueOf(final char[] cbuf, final int offset, final int len) throws NumberFormatException, ArithmeticException {
        if ((cbuf == null) || (len == 0)) {
            return (Short) defaultValue();
        }
        ...
        if (AbstractIntegerType.hasRadixPrefix(cbuf, offset, len)) {
            return Numbers.toShort(new String(cbuf, offset, len));
        }
        ...
        if ((i < Short.MIN_VALUE) || (i > Short.MAX_VALUE)) {
            ...
            // Numbers.toShort throws on the String path; NumberFormatException is reserved for malformed text.
