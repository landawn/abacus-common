...
public class OptionalLongType extends AbstractOptionalType<OptionalLong> {
     ...
     * @return an OptionalLong containing the long value, or empty if the column value is SQL NULL. A non-{@code Number}
     *         column value is coerced with {@link Numbers#toLong(Object)}, so an empty string yields a <i>present</i>
     *         zero (the same answer {@code LongType.get} gives), unlike {@link #valueOf(String)} which answers empty for
     *         {@code ""}
     ...
     * same substitution {@code LongType}, {@code MutableLongType} and {@code AtomicLongType} apply to a {@code null}
     * value. A substituted zero reads back as a <i>present</i> {@code OptionalLong.of(0L)}, which is what the flag asks
    ...
    public void serializeTo(final CharacterWriter writer, final OptionalLong x, final JsonXmlSerConfig<?> config) throws NullPointerException, IOException {
        ...
        if (absent && !(config != null && config.isWriteNullNumberAsZero())) {
            ...
            final long value = absent ? 0L : x.get();
