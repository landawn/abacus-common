...
public final class MutableFloat extends Number implements Comparable<MutableFloat>, Mutable {
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * MutableFloat num = MutableFloat.of(0.0f);
     * }</pre>
     *
     */
    MutableFloat() {
    ...
    /**
     * Gets the value as a primitive float.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * MutableFloat num = MutableFloat.of(42.5f);
     * float val = num.getValue();        // returns 42.5f, value is unchanged
     *
     * num.setValue(7.25f);
     * val = num.getValue();              // returns 7.25f, value is unchanged
     *
     * MutableFloat max = MutableFloat.of(Float.MAX_VALUE);
     * val = max.getValue();              // returns 3.4028235E38f (Float.MAX_VALUE)
     * }</pre>
     *
     * @return the current float value
     ...
     *
     * <p>This method is useful when you need both the old and new values in a single call,
     ...
     * float delta = temperature.value() - previousTemp;  // delta is 0.5f
     * }</pre>
     *
     ...
     * System.out.println("New score: " + score.setAndGet(95.5f));   // prints "New score: 95.5"
     *
     * // Can be used in calculations
     * float result = score.setAndGet(100.0f) * 0.9f;  // value is now 100.0f, result is 90.0f
     * }</pre>
     *
     * @param newValue the new value to set
     ...
     * float change = counter.value() - previous;  // change is 25.5f
     * }</pre>
     *
     * @param delta the value to add to the current value
     ...
     * if (progress.addAndGet(25.0f) >= 100.0f) {
     *     System.out.println("Task complete!");   // Executes: progress reached 100%
     * }
     * }</pre>
     *
     * @param delta the value to add to the current value
    ...
    /**
     * Returns the value of this MutableFloat as an int by truncating the fractional part.
     *
     * <p>The conversion follows standard Java casting rules from float to int:</p>
     * <ul>
     * <li>The fractional part is discarded (truncated towards zero)</li>
     * <li>NaN is converted to 0</li>
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * MutableFloat num = MutableFloat.of(42.7f);
     * int intVal = num.intValue();   // returns 42
     * }</pre>
     *
     * @return the value as an int, with fractional part truncated
     */
    @Override
    public int intValue() {
        return (int) value;
    ...
    /**
     * Returns the value of this MutableFloat as a long by truncating the fractional part.
     *
     * <p>The conversion follows standard Java casting rules from float to long:</p>
     * <ul>
     * <li>The fractional part is discarded (truncated towards zero)</li>
     * <li>NaN is converted to 0</li>
     ...
     * }</pre>
     *
     * @return the value as a long, with fractional part truncated
     */
    @Override
     ...
     * This is an exact representation of the stored value with no conversion.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * MutableFloat num = MutableFloat.of(42.5f);
     * float val = num.floatValue();   // returns 42.5f
     * }</pre>
     *
     ...
     * <pre>{@code
     * MutableFloat num = MutableFloat.of(42.5f);
     * double val = num.doubleValue();   // returns 42.5
     * }</pre>
     *
     ...
     * The comparison is consistent with {@link Float#compare(float, float)} and properly handles
     * NaN and signed zero values according to IEEE 754 standards.
     *
     ...
     *
     * MutableFloat nan = MutableFloat.of(Float.NaN);
     * result = nan.compareTo(MutableFloat.of(Float.POSITIVE_INFINITY));   // returns positive value
     * }</pre>
     *
     ...
     * @return {@code true} if the objects represent the same float value; {@code false} otherwise
     ...
     * <pre>{@code
     * MutableFloat num = MutableFloat.of(42.5f);
     * int hash = num.hashCode();   // returns Float.hashCode(42.5f)
     * }</pre>
     *
