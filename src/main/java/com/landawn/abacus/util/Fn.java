 ...
 * consumers, suppliers, and operators with sophisticated {@code null} handling, type conversions,
 ...
 *   <li><b>Conditional Consumers:</b> Side effects with conditional execution and {@code null} safety</li>
 ...
 *   <li>Stateless functions may be safely reused when their captured delegates are safe to reuse</li>
 *   <li>Methods marked with {@code @Stateful} return functions whose state and concurrency policy are described by the individual method</li>
 * </ul>
 *
 ...
 *   <li><b>Type Conversion:</b> Safe type transformations with {@code null} handling and default values</li>
 ...
 * <ul>
 *   <li><b>Null-Safe Predicates:</b> Handle {@code null} inputs gracefully with defined behavior</li>
 *   <li><b>Default Value Functions:</b> Provide fallback values for {@code null} or invalid inputs</li>
 *   <li><b>Conditional Operations:</b> Execute operations only when inputs meet safety criteria</li>
 *   <li><b>Optional Integration:</b> Seamless integration with Optional for {@code null} handling</li>
 * </ul>
 *
 ...
 *   <li><b>Returned Functions:</b> Adapters that invoke a caller-supplied delegate inherit that delegate's thread-safety</li>
 *   <li><b>Stateful Functions:</b> {@code @Stateful} indicates retained state, not necessarily a lack of thread safety</li>
 ...
 * <ul>
 *   <li>Reuse stateless functions when useful; create a fresh stateful function for each independent operation unless shared state is intentional</li>
 ...
 *   <li>Use null-safe variants when input data might contain {@code null} values</li>
 ...
 *   <li>Handles {@code null} inputs gracefully according to each method's documented behavior</li>
 ...
 *   <li>Memoized functions retain cached keys and values; avoid using them with an unbounded input space</li>
 ...
 *   <li><b>vs Standard Java:</b> Enhanced with {@code null} safety, default values, and convenience operations</li>
...
public final class Fn {
     ...
     * If the function throws, no result is cached and a later invocation with the same input retries the function.
     *
     * <p><b>Memory Considerations:</b> The cache grows unbounded as new inputs are processed.
     * For scenarios where the input space is very large or unbounded, use a bounded or expiring
     * cache designed for functions; the supplier-only expiration overload does not provide
     * per-input caching.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @throws IllegalArgumentException if {@code service} or {@code timeUnit} is {@code null}, or if
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.throwException(IllegalStateException::new).accept(any);   // throws IllegalStateException
     * }</pre>
     *
     * @param <T> the type of the input to the consumer
     ...
     * Reuse the returned consumer for all invocations that should share the specified rate.
     * The underlying {@link RateLimiter} supports concurrent callers.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * Reuse the returned consumer for all invocations that should share the provided limiter.
     * The underlying {@link RateLimiter} supports concurrent callers.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @deprecated replaced by {@link #entryWithKey(Object)}
     ...
     * @param maxValue the upper bound (exclusive)
     * @return a Predicate that tests if minValue &lt; input &lt; maxValue
     * @deprecated replaced by {@link #gtAndLt(Comparable, Comparable)}.
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.acceptByKey(k -> N.println(k)).accept(Map.entry("hello","world")); // prints hello
     * }</pre>
     *
     * @param <K> the key type
     * @param <V> the value type
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.acceptByValue(v -> N.println(v)).accept(Map.entry("k",100));   // prints 100
     * }</pre>
     *
     * @param <K> the key type
     * @param <V> the value type
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.acceptKeyVal((k, v) -> N.println(k + "=" + v)).accept(Map.entry("age",25)); // prints age=25
     * }</pre>
     *
     * @param <K> the key type
     * @param <V> the value type
     ...
     *
     * <p>This is useful for filtering out {@code null} values in stream operations without explicitly
     * checking for {@code null} in your consumer logic.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @return a Consumer that conditionally processes values
     * @throws IllegalArgumentException if any of {@code predicate}, {@code consumerForTrue},
     *         {@code consumerForFalse} is {@code null}.
     * @see #acceptIf(java.util.function.Predicate, java.util.function.Consumer)
     */
    @Beta
     ...
     *
     * <p>This method provides null-safe function composition. If the input is {@code null}, or if any intermediate
     * mapper in the chain returns {@code null}, the default value is returned; a {@code null} result from the final mapper
     * is returned as-is.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    /**
     * Returns a Function that transposes a Map with Collection values into a List of Maps.
     * Each output Map contains, for every key, the element at the same index in that key's
     * collection: the first map holds the first elements of all collections, the second map
     * holds the second elements, and so on.
     *
     ...
     * @param <K> the key type
     * @param <V> the value type
     * @return a Function that transposes Maps with Collection values
    ...
    /**
     * Returns a stateful Predicate that limits the number of invocations that pass through.
     * The first <i>count</i> calls return {@code true}; all subsequent calls return {@code false}.
     *
     * <p>This predicate is stateful and maintains an atomic counter, but should not be saved or cached
     * for reuse across multiple streams. Concurrent calls cannot exceed the limit; in a parallel stream,
     * however, the accepted elements are selected by invocation order and are not guaranteed to be the
     * first <i>count</i> elements in encounter order.
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * </ol>
     *
     * <p>In sequential use, this is useful when you only want to test the first N elements of a stream
     * and filter those N elements according to some condition.
     *
     ...
     * }</pre>
     *
     * <p><b>Concurrency:</b> The atomic counter prevents more than {@code limit} invocations from
     * reaching the predicate, but a parallel stream does not guarantee which encounter-order elements
     * consume those slots. The supplied predicate must itself support concurrent invocation.
     *
     * @param <T> the type of the input to the predicate
    ...
    /**
     * Returns a stateful {@code BiPredicate}. Don't save or cache it for reuse. Concurrent calls cannot
    ...
    /**
     * Returns a stateful {@code Predicate}. Don't save or cache it for reuse. Concurrent calls cannot
     * accept more than {@code limit} matches, but which matching calls are accepted is scheduling-dependent
     * and the supplied predicate must itself support concurrent invocation.
     *
     ...
     * @see #futureGetOrDefaultOnError(Object)
     */
    @SuppressWarnings("rawtypes")
    @Beta
     ...
     * <ul>
     *   <li>{@code s()} / {@code ss()} — <b>S</b>upplier (plain / from a Throwables.Supplier)</li>
     *   <li>{@code p()} / {@code pp()} — <b>P</b>redicate / BiPredicate / TriPredicate (plain / from the Throwables variants)</li>
     *   <li>{@code c()} / {@code cc()} — <b>C</b>onsumer / BiConsumer / TriConsumer, also Callable (plain / from the Throwables variants)</li>
     *   <li>{@code f()} / {@code ff()} — <b>F</b>unction / BiFunction / TriFunction (plain / from the Throwables variants)</li>
     *   <li>{@code o()} — Unary/Binary <b>O</b>perator identity</li>
     *   <li>{@code mc()} — <b>m</b>apMulti-style Bi<b>C</b>onsumer mapper identity</li>
     *   <li>{@code sp()} / {@code sc()} / {@code sf()} — <b>s</b>ynchronized <b>P</b>redicate / <b>C</b>onsumer / <b>F</b>unction</li>
    ...
    /** Shared, stateless function that unwraps an abacus {@link Optional}, yielding {@code null} when it is empty. */
    @SuppressWarnings("rawtypes")
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.sp(new Object(), (String s) -> s.length() > 3).test("hello");   // returns true
     * }</pre>
     *
     * @param <T> the type of the input to the predicate
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.sp(new Object(), "hello", (String a, String b) -> a.equals(b)).test("hello");   // returns true
     * }</pre>
     *
     * @param <A> the type of the fixed first argument to the bi-predicate
     * @param <T> the type of the input to the resulting predicate
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.sp(new Object(), (String a, Integer b) -> a.length() == b).test("abc", 3);   // returns true
     * }</pre>
     *
     * @param <T> the type of the first input to the bi-predicate
     * @param <U> the type of the second input to the bi-predicate
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.sp(new Object(), (Integer a, Integer b, Integer c) -> a + b == c).test(1, 2, 3);   // returns true
     * }</pre>
     *
     * @param <A> the type of the first input to the tri-predicate
     * @param <B> the type of the second input to the tri-predicate
     ...
     * @throws IllegalArgumentException if {@code mutex} is {@code null}, or if {@code triPredicate} is {@code null}.
     * @see #sp(Object, java.util.function.Predicate)
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.sc(new Object(), (String s) -> System.out.println(s)).accept("hello");   // prints hello
     * }</pre>
     *
     * @param <T> the type of the input to the consumer
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * Fn.sc(new Object(), "[", (String pre, String s) -> System.out.println(pre + s)).accept("hi");   // prints [hi
     * }</pre>
     *
     * @param <A> the type of the fixed first argument to the bi-consumer
     * @param <T> the type of the input to the resulting consumer
     ...
     * @throws IllegalArgumentException if {@code mutex} is {@code null}, or if {@code triConsumer} is {@code null}.
     * @see #sc(Object, java.util.function.Consumer)
     ...
     * @throws IllegalArgumentException if {@code mutex} is {@code null}, or if {@code triFunction} is {@code null}.
     * @see #sf(Object, java.util.function.Function)
    ...
    /**
     * <b>{@code cc}</b> stands for <b>Throwable Callable</b> (in this overload; converts checked exceptions).
     ...
     * merger.apply("a", "b");                                    // throws IllegalStateException
     * }</pre>
     *
     * @param <T> the type of the operands and result
    ...
    /** Shared, stateless function that unwraps a JDK {@link java.util.Optional}, yielding {@code null} when it is empty. */
    @SuppressWarnings("rawtypes")
    ...
    /** Shared, stateless predicate that reports whether an abacus {@link Optional} holds a value. */
    @SuppressWarnings("rawtypes")
    ...
    /** Shared, stateless predicate that reports whether a JDK {@link java.util.Optional} holds a value. */
    @SuppressWarnings("rawtypes")
    ...
    public static final class FC {
        ...
        /**
         * Returns a CharPredicate that tests if a character is the {@code null} character.
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
