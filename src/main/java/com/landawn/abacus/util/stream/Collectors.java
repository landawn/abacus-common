...
import com.landawn.abacus.util.BinaryOperators;
 ...
 *   <li><b>Functional Composition:</b> Teeing and combining collectors for single-pass aggregation</li>
 *   <li><b>Performance Optimization:</b> Specialized collectors optimized for specific data types</li>
 *   <li><b>Null Handling:</b> Collector-specific behavior documented by each factory method</li>
 * </ul>
 *
 ...
 * }</pre>
 *
 * <p><b>&#9888;&#65039; What "encounter order" means in this class.</b> Many collectors below are
 * documented as collecting "in encounter order". That describes <b>sequential</b> execution. On an
 * abacus parallel stream ({@link com.landawn.abacus.util.stream.Stream#parallel() Stream.parallel()})
 * encounter order is <b>not</b> preserved: elements are handed to worker threads one at a time and
 * results are emitted in completion order, so a collector's output is an unspecified permutation
 * &mdash; this differs from the JDK, whose parallel streams do preserve encounter order for ordered
 * sources. Where a collector's own text describes <i>how</i> partial results are combined (for example
 * "by appending the later segment to the earlier one"), that describes the combiner, not a guarantee
 * that the segments arrive in source order.
 *
 ...
 *   <li><b>Multimaps:</b> {@code toMultimap()}, {@code flatMappingValueToMultimap()}, {@code flatMappingKeyToMultimap()}</li>
 ...
 * </ul>
 *
 * <p><b>Immutable Collection Benefits:</b></p>
 * <ul>
 ...
 * </ul>
 *
 * <p><b>Advanced Grouping Features:</b></p>
 * <ul>
 ...
 * </ul>
 *
 * <p><b>BiMap Collector Features:</b></p>
 * <ul>
 ...
 * </ul>
 *
 * <p><b>Multimap Collector Options:</b></p>
 * <ul>
 ...
 * </ul>
 *
 * <p><b>Statistical Collectors:</b></p>
 * <ul>
 *   <li><b>Primitive Statistics:</b> int, long, double with count, sum, min, max, average</li>
 *   <li><b>BigDecimal/BigInteger:</b> Arbitrary-precision sums and decimal averages</li>
 ...
 * <ul>
 *   <li><b>Collector Instances:</b> Returned collectors are reusable when their supplied functions,
 *       factories, and downstream collectors satisfy the {@link Collector} contract</li>
 *   <li><b>Concurrent Accumulation:</b> Only collectors declaring
 *       {@link Characteristics#CONCURRENT CONCURRENT} permit multiple threads to update one
 *       accumulation container; other parallel collectors rely on isolated partial containers</li>
 *   <li><b>Result Containers:</b> A result is thread-safe only when its documented result type or
 *       supplied factory provides that guarantee</li>
 *   <li><b>Parallel Streams:</b> Most collectors support parallel reduction; sequential-only
 *       collectors are identified in their method documentation</li>
 * </ul>
 *
 * <p><b>Error Handling:</b></p>
 * <ul>
 *   <li>Validates required parameters when collectors are created; exception types are documented per method</li>
 *   <li>Throws {@link IllegalStateException} for constraint violations (e.g., duplicate keys)</li>
 *   <li>Handles {@code null} elements according to the specific collector's documented behavior</li>
 *   <li>Provides clear error messages for debugging collection failures</li>
 * </ul>
 *
 * <p><b>Integration with Standard Collectors:</b></p>
 * <ul>
 ...
 * <ul>
 *   <li>Use primitive-result collectors when collecting numeric values from object streams</li>
 ...
 *   <li>Consider bounded or batched processing when all results should not be retained in memory</li>
 * </ul>
 *
 * <p><b>Comparison with Standard Collectors:</b></p>
 * <ul>
 ...
 *   <li><b>Specialized Implementations:</b> Purpose-built collectors for additional result types</li>
 *   <li><b>Null Handling:</b> Behavior is documented for each collector</li>
 * </ul>
 *
...
public abstract sealed class Collectors permits Collectors.MoreCollectors { // NOSONAR
    ...
    /**
     * Converts the specified {@code characteristics} collection into the array expected by
     * {@link Collector#of(Supplier, BiConsumer, BinaryOperator, Characteristics...)}.
     *
     * <p>A {@code null} element is rejected here with an {@link IllegalArgumentException} naming its position,
     * rather than being allowed to surface later as a bare {@link NullPointerException} thrown from inside
     * {@code EnumSet} while {@code Collector.of} builds its characteristics set.
     *
     * @param characteristics the characteristics to convert; may be {@code null} or empty
     * @return {@link #CH_NOID} if {@code characteristics} is {@code null} or empty, otherwise its elements as an array
     * @throws IllegalArgumentException if {@code characteristics} contains a {@code null} element
     */
    private static Characteristics[] toCharacteristicsArray(final Collection<Characteristics> characteristics) throws IllegalArgumentException {
        if (N.isEmpty(characteristics)) {
            return CH_NOID;
        }

        final Characteristics[] a = characteristics.toArray(Characteristics[]::new);
        checkCharacteristics(a);

        return a;
    }

    /**
     * @throws IllegalArgumentException if {@code characteristics} or any of its elements is {@code null}.
     */
    private static void checkCharacteristics(final Characteristics[] characteristics) throws IllegalArgumentException {
        N.checkArgNotNull(characteristics, cs.characteristics);

        for (int i = 0, len = characteristics.length; i < len; i++) {
            N.checkArgNotNull(characteristics[i], "characteristics[" + i + "]");
        }
    }
     ...
     * @param keyMapper the classifier function to wrap
     ...
     *         {@code characteristics} is {@code null}, or if {@code characteristics} contains a {@code null} element
    ...
    public static <T, R> Collector<T, R, R> create(final Supplier<? extends R> supplier, final BiConsumer<? super R, ? super T> accumulator,
            final BinaryOperator<R> combiner, final Characteristics... characteristics) throws IllegalArgumentException {
        N.checkArgNotNull(supplier, cs.supplier);
        N.checkArgNotNull(accumulator, cs.accumulator);
        N.checkArgNotNull(combiner, cs.combiner);
     ...
     * Set<Collector.Characteristics> chars = EnumSet.of(Collector.Characteristics.IDENTITY_FINISH);
     * Collector<String, StringBuilder, StringBuilder> collector = Collectors.create(
     *     StringBuilder::new,
     *     (sb, s) -> sb.append(s),
     *     (sb1, sb2) -> sb1.append(sb2),
     ...
     * @param accumulator the accumulator function that folds a value into a mutable result container
     * @param combiner the combiner function that merges two result containers
     * @param characteristics optional characteristics of the collector; may be {@code null} or empty, but must not
     *        contain a {@code null} element
     * @return a new {@code Collector} with the specified supplier, accumulator, and combiner
     * @throws IllegalArgumentException if any of {@code supplier}, {@code accumulator}, or {@code combiner} is
     *         {@code null}, or if {@code characteristics} contains a {@code null} element.
     * @see Collector#of(Supplier, BiConsumer, BinaryOperator, Characteristics...)
     */
    public static <T, R> Collector<T, R, R> create(final Supplier<? extends R> supplier, final BiConsumer<? super R, ? super T> accumulator,
            final BinaryOperator<R> combiner, final Collection<Characteristics> characteristics) throws IllegalArgumentException {
        N.checkArgNotNull(supplier, cs.supplier);
        N.checkArgNotNull(accumulator, cs.accumulator);
        N.checkArgNotNull(combiner, cs.combiner);

        return Collector.of((Supplier<R>) supplier, (BiConsumer<R, T>) accumulator, combiner, toCharacteristicsArray(characteristics));
    }
     ...
     * @throws IllegalArgumentException if any of {@code supplier}, {@code accumulator}, {@code combiner},
     *         {@code finisher}, or {@code characteristics} is {@code null}, or if {@code characteristics} contains a {@code null} element
     ...
     * @return a new {@code Collector} with the specified components
     * @throws IllegalArgumentException if any of {@code supplier}, {@code accumulator}, {@code combiner}, or
     *         {@code finisher} is {@code null}, or if {@code characteristics} contains a {@code null} element.
     * @see Collector#of(Supplier, BiConsumer, BinaryOperator, Function, Characteristics...)
     */
    public static <T, A, R> Collector<T, A, R> create(final Supplier<? extends A> supplier, final BiConsumer<? super A, ? super T> accumulator,
        ...
        return Collector.of((Supplier<A>) supplier, (BiConsumer<A, T>) accumulator, combiner, (Function<A, R>) finisher,
                toCharacteristicsArray(characteristics));
    }
     ...
     *
     * <p><b>Null Handling:</b> The collector allows {@code null} elements if the underlying collection
     ...
     * @see #toSet()
     */
    public static <T, C extends Collection<T>> Collector<T, ?, C> toCollection(final Supplier<? extends C> collectionFactory) throws IllegalArgumentException {
        N.checkArgNotNull(collectionFactory, cs.collectionFactory);

        final BiConsumer<C, T> accumulator = BiConsumers.ofAdd();
        // Left-preserving: a bigger-wins combiner reorders segments under JDK parallel streams,
        // breaking the documented encounter order.
        final BinaryOperator<C> combiner = BinaryOperators.ofAddAllToFirst();
     ...
     *
     * <p>When combining collections in parallel processing, if the collection holding the
     * later segment is a {@code List}, the combiner copies the needed prefix of it via
     * {@code subList} for efficiency. For other collection types, it iterates through its
     * elements until the size limit is reached.</p>
     *
     * <p><b>Null Handling:</b> The collector allows {@code null} elements if the underlying collection
     * supports {@code null} values. Null elements count toward the size limit.</p>
     *
    ...
    public static <T, C extends Collection<T>> Collector<T, ?, C> toCollection(final int atMostSize, final Supplier<? extends C> collectionFactory)
            throws IllegalArgumentException {
        N.checkArgNotNegative(atMostSize, cs.atMostSize);
        N.checkArgNotNull(collectionFactory, cs.collectionFactory);
     ...
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams with
     * a default combiner that appends the later partial collection to the earlier one.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * implementations. These functions are responsible for handling {@code null} elements and null
     ...
     */
    public static <T, C extends Collection<T>> Collector<T, ?, C> toCollection(final Supplier<? extends C> supplier, final BiConsumer<C, T> accumulator,
            final BinaryOperator<C> combiner) throws IllegalArgumentException {
        N.checkArgNotNull(supplier, cs.supplier);
        N.checkArgNotNull(accumulator, cs.accumulator);
        N.checkArgNotNull(combiner, cs.combiner);
    ...
    /**
     * Internal delegate to {@link #create(Supplier, BiConsumer, BinaryOperator, Characteristics...)}.
     *
     * <p><b>Not public API.</b> This overload is package-private and cannot be called from outside
     * {@code com.landawn.abacus.util.stream}. Callers that need to supply their own
     * {@link Characteristics} should use
     * {@link #create(Supplier, BiConsumer, BinaryOperator, Characteristics...)} directly; the public
     * {@code toCollection} overloads cover the cases that do not need explicit characteristics.
     *
     * @param <T> the type of input elements
     * @param <C> the type of the resulting collection
     ...
     * @param characteristics optional characteristics of the collector
     * @return a {@code Collector} which collects input elements into a collection
     * @throws IllegalArgumentException if any of {@code supplier}, {@code accumulator}, {@code combiner}, or {@code characteristics}
     *         is {@code null}, or {@code characteristics} contains a {@code null} element
     * @see #create(Supplier, BiConsumer, BinaryOperator, Characteristics...)
     * @see #toCollection(Supplier, BiConsumer, BinaryOperator)
     */
    static <T, C extends Collection<T>> Collector<T, ?, C> toCollection(final Supplier<? extends C> supplier, final BiConsumer<C, T> accumulator,
            final BinaryOperator<C> combiner, final Characteristics... characteristics) throws IllegalArgumentException {
     ...
     *
     * <p><b>Null Handling:</b> This collector accepts {@code null} elements. The resulting list
     * will contain {@code null} values in the positions where they were encountered in the stream.</p>
     *
     ...
     *
     * <p><b>Null Handling:</b> This collector accepts {@code null} elements as {@code LinkedList}
     * supports {@code null} values.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams and
     * efficiently combines partial results.</p>
     *
     ...
     *
     * <p><b>Null Handling:</b> This collector accepts {@code null} elements if they are present
     * in the stream. The resulting immutable list will contain {@code null} values.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams and
     * efficiently combines partial results.</p>
     *
     ...
     *
     * <p><b>Null Handling:</b> This collector does not accept {@code null} elements. If the stream
     * contains any {@code null} values, a {@code NullPointerException} will be thrown.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams and
     * efficiently combines partial results.</p>
     *
     ...
     * is used).</p>
     *
     * <p><b>Null Handling:</b> This collector accepts {@code null} elements if the underlying
     * set implementation supports {@code null} values.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams and
    ...
    public static <T> Collector<T, ?, Set<T>> toLinkedHashSet() {
        ...
        final BiConsumer<Set<T>, T> accumulator = BiConsumers.ofAdd();
        // Left-preserving: a bigger-wins combiner reorders segments under JDK parallel streams,
        // breaking the insertion-order contract.
        final BinaryOperator<Set<T>> combiner = BinaryOperators.ofAddAllToFirst();
     ...
     *
     * <p><b>Null Handling:</b> This collector accepts {@code null} elements as the underlying
     * {@code Queue} implementation supports {@code null} values.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams and
     * efficiently combines partial results.</p>
     *
     ...
     *
     * <p><b>Null Handling:</b> This collector accepts {@code null} elements as the underlying
     * {@code Deque} implementation supports {@code null} values.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams and
     * efficiently combines partial results.</p>
     *
     ...
     */
    public static <T> Collector<T, ?, List<T>> toList(final int atMostSize) throws IllegalArgumentException {
     ...
     * @throws IllegalArgumentException if {@code atMostSize} is negative.
     */
    public static <T> Collector<T, ?, Set<T>> toSet(final int atMostSize) throws IllegalArgumentException {
        final Supplier<Set<T>> supplier = () -> N.newHashSet(N.min(256, atMostSize));
     ...
     *     .collect(Collectors.toMultiset());             // returns a Multiset; count("apple") is 3
     *
     ...
     *     .collect(Collectors.toMultiset(Multiset::new)); // returns a Multiset; count(3) is 3
     *
     ...
     * assignment-compatible with the supplied array's component type.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link ArrayStoreException} if an element is not assignable to the supplied array's component type.
     * The finisher throws {@link NullPointerException} if {@code arraySupplier} returns {@code null}.</p>
     *
     * @param <T> the type of input elements
    ...
    public static <T, A> Collector<T, ?, A[]> toArray(final Supplier<A[]> arraySupplier) throws IllegalArgumentException {
        ...
        // Left-preserving: a bigger-wins combiner reorders segments under JDK parallel streams,
        // breaking the documented encounter order.
        final BinaryOperator<List<A>> combiner = BinaryOperators.ofAddAllToFirst();
     ...
     * you know the component type of the array at compile time. Every collected element
     * must be assignment-compatible with the returned array's component type.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link ArrayStoreException} if an element is not assignable to the returned array's component type.
     * The finisher throws {@link NullPointerException} if {@code arraySupplier} returns {@code null}.</p>
     *
     * @param <T> the type of input elements
     * @param <A> the component type of the array
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link TooManyElementsException} if more than one element is collected.</p>
     *
     * @param <T> the type of input elements
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link TooManyElementsException} if more than one element matches the predicate.</p>
     *
     * @param <T> the type of input elements
    ...
    private static final BinaryOperator<Holder<Object>> first_last_combiner = (t, u) -> {
        if (t.value() != NONE && u.value() != NONE) {
            throw new UnsupportedOperationException("The 'first' and 'last' collectors can only be used with sequential streams"); //NOSONAR
        }
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link UnsupportedOperationException} if a reduction combines two nonempty partial results.</p>
     *
     * @param <T> the type of input elements
     ...
     * collector is designed for sequential streams only. Its combiner throws an
     * {@code UnsupportedOperationException} when two nonempty partial results are combined,
     * so parallel use is unsupported even though an empty or otherwise unsplit parallel
     * reduction might happen to complete.</p>
     *
     * <p>Once {@code n} elements have been collected, subsequent elements are ignored.
     * As with any collector, this does not short-circuit traversal of the source.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T> Collector<T, ?, List<T>> first(final int n) throws IllegalArgumentException {
        ...
        final BinaryOperator<List<T>> combiner = (a, b) -> {
            if (N.notEmpty(a) && N.notEmpty(b)) {
                throw new UnsupportedOperationException("The 'first' and 'last' collectors can only be used with sequential streams");
            }
     ...
     * It uses a {@code LinkedList}-based {@code Deque} internally for efficient addition
     * and removal of elements at both ends, which also allows {@code null} elements to be
     * collected.</p>
     *
     * <p>This collector is designed for sequential streams only. Its combiner throws an
     * {@code UnsupportedOperationException} when two nonempty partial results are combined,
     * so parallel use is unsupported even though an empty or otherwise unsplit parallel
     * reduction might happen to complete.</p>
     *
    ...
    public static <T> Collector<T, ?, List<T>> last(final int n) throws IllegalArgumentException {
        ...
        // LinkedList for null tolerance: ArrayDeque rejects null elements, which the sibling first(n) accepts.
        final Supplier<Deque<T>> supplier = LinkedList::new;
     ...
     */
    public static Collector<Object, ?, String> joining(final CharSequence delimiter) throws IllegalArgumentException {
     ...
     *
     * <p>The delimiter, prefix, and suffix are converted to immutable {@code String}
     * snapshots when this method is called. Later mutations of a supplied mutable
     * {@code CharSequence} therefore do not change the collector's formatting.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static Collector<Object, ?, String> joining(final CharSequence delimiter, final CharSequence prefix, final CharSequence suffix)
        ...
        // Snapshot mutable CharSequence arguments once. Every parallel accumulation container
        // must use the same formatting configuration even if a caller later mutates an argument.
        final String delimiterString = delimiter.toString();
        final String prefixString = prefix.toString();
        final String suffixString = suffix.toString();

        @SuppressWarnings("resource")
        final Supplier<Joiner> supplier = () -> Joiner.with(delimiterString, prefixString, suffixString).reuseBuffer();
     ...
     */
    @Beta
    public static <T> Collector<T, ?, List<T>> filteringToList(final Predicate<? super T> predicate) throws IllegalArgumentException {
        N.checkArgNotNull(predicate, cs.predicate);
     ...
     */
    @Beta
    public static <T, U> Collector<T, ?, List<U>> mappingToList(final Function<? super T, ? extends U> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * their elements have been consumed. A {@code null} stream is treated as empty.
     * This ensures proper resource management.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T, U, A, R> Collector<T, ?, R> flatMapping(final Function<? super T, ? extends Stream<? extends U>> mapper,
            final Collector<? super U, A, R> downstream) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
        N.checkArgNotNull(downstream, cs.downstream);
     ...
     *
     * <p>Null or empty collections returned by the mapper are treated as empty and don't
     ...
     * collector, with only {@code IDENTITY_FINISH} removed. In particular, a concurrent
     * downstream collector remains concurrent because the additional finisher runs only
     * after accumulation has completed.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T, A, R, RR> Collector<T, A, RR> collectingAndThen(final Collector<T, A, R> downstream, final Function<? super R, RR> finisher)
        ...
        if (characteristics.contains(Characteristics.IDENTITY_FINISH)) {
            ...
            characteristics = characteristics.isEmpty() ? N.emptySet() : Collections.unmodifiableSet(characteristics);
        }
     ...
     * place {@link #filtering(Predicate, Collector)} outside this collector.</p>
     *
     * <p>The returned collector inherits the downstream collector's characteristics except
     * {@link Characteristics#IDENTITY_FINISH IDENTITY_FINISH} and
     * {@link Characteristics#CONCURRENT CONCURRENT}, which are always removed: an extra
     * finisher is applied, and the seen-an-element flag is tracked per accumulation container.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T, A, R> Collector<T, ?, Optional<R>> collectingOrEmpty(final Collector<T, A, R> collector) throws IllegalArgumentException {
        ...
        final BinaryOperator<A> downstreamCombiner = collector.combiner();
        final Function<A, R> downstreamFinisher = collector.finisher();

        // The accumulated-flag must live in the accumulation container, not in the collector:
        // a collector can be reused, and groupingBy creates one container per group.
        final Supplier<Pair<A, MutableBoolean>> supplier = () -> Pair.of(downstreamSupplier.get(), MutableBoolean.of(false));

        final BiConsumer<Pair<A, MutableBoolean>, T> newAccumulator = (a, t) -> {
            downstreamAccumulator.accept(a.left(), t);
            a.right().setTrue();
        };

        final BinaryOperator<Pair<A, MutableBoolean>> newCombiner = (a, b) -> Pair.of(downstreamCombiner.apply(a.left(), b.left()),
                MutableBoolean.of(a.right().isTrue() || b.right().isTrue()));
        ...
        if (characteristics.contains(Characteristics.IDENTITY_FINISH) || characteristics.contains(Characteristics.CONCURRENT)) {
            characteristics = EnumSet.copyOf(characteristics);
            characteristics.remove(Characteristics.IDENTITY_FINISH);
        ...
        return create(supplier, newAccumulator, newCombiner, newFinisher, characteristics);
    }
     ...
     * @throws IllegalArgumentException if {@code collector} or {@code defaultForEmpty} is {@code null}.
     */
    @Beta
    public static <T, A, R> Collector<T, ?, R> collectingOrElseGetIfEmpty(final Collector<T, A, R> collector, final Supplier<? extends R> defaultForEmpty)
            throws IllegalArgumentException {
        N.checkArgNotNull(collector, cs.collector);
        N.checkArgNotNull(defaultForEmpty, cs.defaultForEmpty);
        ...
        final Function<Pair<A, MutableBoolean>, R> newFinisher = a -> {
            if (a.right().isTrue()) {
                return downstreamFinisher.apply(a.left());
            } else {
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link NoSuchElementException} if no elements are collected.</p>
     *
     * @param <T> the type of the input elements
     * @param <A> intermediate accumulation type of the collector
     * @param <R> result type of the collector
     ...
     */
    @Beta
    public static <T, A, R> Collector<T, ?, R> collectingOrElseThrowIfEmpty(final Collector<T, A, R> collector) throws IllegalArgumentException {
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws the exception supplied by {@code exceptionSupplier} if no elements are collected. If {@code exceptionSupplier} returns {@code null}, throwing it causes {@link NullPointerException}.</p>
     *
     * @param <T> the type of the input elements
     * @param <A> intermediate accumulation type of the collector
     * @param <R> result type of the collector
     ...
     *
     * <p>For ordered streams, the first occurrence for each key is passed to the supplied
     * collection in encounter order. The final iteration order and any additional duplicate
     * elimination still follow the semantics of that collection (for example, a
     * {@code TreeSet} sorts its elements).</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T, C extends Collection<T>> Collector<T, ?, C> distinctByToCollection(final Function<? super T, ?> keyMapper,
        ...
        // Keeps the FIRST occurrence per key in encounter order (LinkedHashMap + left-preserving
        // combiner) — must NOT be UNORDERED, matching the documented first-occurrence contract.
        return create(mappSupplier, accumulator, combiner, finisher, CH_NOID);
    }
     ...
     */
    @SuppressWarnings("rawtypes")
    public static <T> Collector<T, ?, Optional<T>> minBy(final Function<? super T, ? extends Comparable> keyMapper) throws IllegalArgumentException {
        N.checkArgNotNull(keyMapper, cs.keyMapper);
     ...
     */
    @SuppressWarnings("rawtypes")
    public static <T> Collector<T, ?, Optional<T>> maxBy(final Function<? super T, ? extends Comparable> keyMapper) throws IllegalArgumentException {
        N.checkArgNotNull(keyMapper, cs.keyMapper);
     ...
     *
     * // Find all strings with the minimum length
     ...
     * @see #minAll()
     */
    public static <T> Collector<T, ?, List<T>> minAll(final Comparator<? super T> comparator) throws IllegalArgumentException {
        N.checkArgNotNull(comparator, cs.comparator);
     ...
     *
     * <p>Comparator-equal elements are retained in encounter order. Consequently, the returned
     * collector does not have the {@link Characteristics#UNORDERED UNORDERED} characteristic.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @throws IllegalArgumentException if {@code downstream} is {@code null}
     * @see #minAll(Comparator, Collector)
     * @see #minAll(Comparator)
     * @see #minAll()
     */
    @SuppressWarnings("rawtypes")
    public static <T extends Comparable, R> Collector<T, ?, R> minAll(final Collector<? super T, ?, R> downstream) throws IllegalArgumentException {
        return minAll(Comparators.<T> nullsLast((a, b) -> a.compareTo(b)), downstream);
    }
     ...
     *
     * <p>The returned collector is unordered only when the downstream collector is unordered;
     * otherwise comparator-equal elements retain the downstream collector's encounter-order requirements.</p>
     *
     * <p>Whenever a strictly smaller element is encountered, a fresh downstream accumulation
     * container is obtained from the downstream supplier. This permits downstream collectors
     * whose accumulation state cannot be reset by clearing collection or map elements.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @see #minAll()
     */
    public static <T, R> Collector<T, ?, R> minAll(final Comparator<? super T> comparator, final Collector<? super T, ?, R> downstream)
            throws IllegalArgumentException {
        N.checkArgNotNull(comparator, cs.comparator);
    ...
    @Beta
    @SuppressWarnings("rawtypes")
    public static <T extends Comparable, R> Collector<T, ?, Optional<Pair<T, R>>> minAllWith(final Collector<? super T, ?, R> downstream)
            throws IllegalArgumentException {
        return minAllWith(Comparators.<T> nullsLast(), downstream);
    }
     ...
     *
     * <p>If several minimal elements compare equal, the pair contains the first encountered one.
     * Consequently, the returned collector does not have the
     * {@link Characteristics#UNORDERED UNORDERED} characteristic.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * @see #maxAll()
     */
    public static <T> Collector<T, ?, List<T>> maxAll(final Comparator<? super T> comparator) throws IllegalArgumentException {
        N.checkArgNotNull(comparator, cs.comparator);
    ...
    public static <T> Collector<T, ?, List<T>> maxAll(final Comparator<? super T> comparator, final int atMostSize) throws IllegalArgumentException {
        ...
        // Comparator-equal elements retain encounter order in the result list.
        return create(supplier, accumulator, combiner, finisher, CH_NOID);
    }
     ...
     */
    @SuppressWarnings("rawtypes")
    public static <T extends Comparable, R> Collector<T, ?, R> maxAll(final Collector<? super T, ?, R> downstream) throws IllegalArgumentException {
        return maxAll(Comparators.<T> nullsFirst(), downstream);
    }
     ...
     * otherwise comparator-equal elements retain the downstream collector's encounter-order requirements.</p>
     *
     * <p>Whenever a strictly greater element is encountered, a fresh downstream accumulation
     * container is obtained from the downstream supplier. This permits downstream collectors
     * whose accumulation state cannot be reset by clearing collection or map elements.</p>
     *
     * <p><b>Usage Examples:</b></p>
    ...
    public static <T, R> Collector<T, ?, R> maxAll(final Comparator<? super T> comparator, final Collector<? super T, ?, R> downstream)
        ...
        final Supplier<Pair<T, Object>> supplier = () -> Pair.of((T) none(), downstreamSupplier.get());
        ...
        final BiConsumer<Pair<T, Object>, T> accumulator = (a, t) -> {
            if (a.left() == NONE) {
                ...
                if (cmp > 0) {
                    // Downstream accumulation containers are opaque. clear() is not guaranteed to
                    // restore state outside Collection/Map elements, so obtain a fresh identity.
        ...
        // Selecting the maximum is order-insensitive, but accumulating comparator-equal
        // elements is only unordered when the downstream collector is unordered.
        final Characteristics[] characteristics = downstream.characteristics().contains(Characteristics.UNORDERED) ? CH_UNORDERED_NOID : CH_NOID;
    ...
    @Beta
    @SuppressWarnings("rawtypes")
    public static <T extends Comparable, R> Collector<T, ?, Optional<Pair<T, R>>> maxAllWith(final Collector<? super T, ?, R> downstream)
            throws IllegalArgumentException {
        return maxAllWith(Comparators.<T> nullsFirst((a, b) -> a.compareTo(b)), downstream);
    }
     ...
     * of the final result.</p>
     *
     * <p>If several maximal elements compare equal, the pair contains the first encountered one.
     * Consequently, the returned collector does not have the
     * {@link Characteristics#UNORDERED UNORDERED} characteristic.</p>
     *
     * <p>Whenever a strictly greater element is encountered, a fresh downstream accumulation
    ...
    public static <T, R, RR> Collector<T, ?, RR> maxAllWith(final Comparator<? super T> comparator, final Collector<? super T, ?, R> downstream,
        ...
        final BiConsumer<Pair<T, Object>, T> accumulator = (a, t) -> {
            if (a.left() == NONE) {
                ...
                if (cmp > 0) {
                    // See maxAll(comparator, downstream): a fresh downstream identity is required.
                    a.setRight(downstreamSupplier.get());
                    a.setLeft(t);
                }
        ...
        // The representative maximum stored in Pair.left is the first comparator-equal element,
        // so the result remains encounter-order-sensitive even with an unordered downstream.
        return create(supplier, accumulator, combiner, finalFinisher, CH_NOID);
    }
     ...
     * natural ordering. Null values are ignored if the stream contains any {@code non-null} element.
     * If the stream is empty, an empty {@code Optional} is returned. If all elements are
     * {@code null}, an {@code Optional} containing {@code Pair.of(null, null)} is returned.</p>
     *
     ...
     */
    public static <T extends Comparable<? super T>> Collector<T, ?, Optional<Pair<T, T>>> minMax() {
        return naturalMinMax(a -> a.present ? Optional.of(Pair.of(a.min, a.max)) : Optional.empty());
    }
     ...
     * provided comparator. If the stream is empty, an empty {@code Optional} is returned.
     * Null input elements are passed to the comparator, so a null-aware comparator can select
     * {@code null} as a legitimate minimum or maximum.</p>
     *
     * <p>The collector produces a stable result for ordered streams: if several
     * minimal or maximal elements appear, the collector always selects the
     * first encountered.</p>
     ...
     * is empty. Null values are ignored if the stream contains any {@code non-null} element. If
     * all elements are {@code null}, {@code Pair.of(null, null)} is returned.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T extends Comparable<? super T>> Collector<T, ?, Pair<T, T>> minMaxOrElseGet(final Supplier<Pair<? extends T, ? extends T>> supplierForEmpty)
        ...
        return naturalMinMax(a -> a.present ? Pair.of(a.min, a.max) : (Pair<T, T>) supplierForEmpty.get());
    }
     ...
     * Null input elements are passed to the comparator, so a null-aware comparator can select
     * {@code null} as a legitimate minimum or maximum without invoking the fallback.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     *         Comparator.comparing(String::length),
     *         () -> Pair.of("", "default")));
     * // result is (empty string, "default")
     * }</pre>
     *
     * @param <T> the type of input elements
     * @param comparator comparator used to compare elements
    ...
    public static <T> Collector<T, ?, Pair<T, T>> minMaxOrElseGet(final Comparator<? super T> comparator,
        ...
        return minMax(comparator, a -> a.present ? Pair.of(a.min, a.max) : (Pair<T, T>) supplierForEmpty.get());
    }
     ...
     * in your application logic. Null values are ignored if the stream contains any
     * {@code non-null} element. If all elements are {@code null}, {@code Pair.of(null, null)}
     * is returned.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T extends Comparable<? super T>> Collector<T, ?, Pair<T, T>> minMaxOrElseThrow() {
        return naturalMinMax(a -> {
            if (a.present) {
                return Pair.of(a.min, a.max);
            } else {
                throw noSuchElementExceptionSupplier.get();
            }
        });
     ...
     *
     * <p>This collector finds both extremes according to the given comparator.
     * If the stream is empty, a {@code NoSuchElementException} is thrown. Null input elements
     * are passed to the comparator, so a null-aware comparator can select {@code null} as a
     * legitimate minimum or maximum.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    private static <T, R> Collector<T, ?, R> minMax(final Comparator<? super T> comparator, final Function<MinMaxHolder<T>, R> finisher) {

        final Supplier<MinMaxHolder<T>> supplier = () -> new MinMaxHolder<>(comparator);

        return create(supplier, MinMaxHolder::accept, MinMaxHolder::combine, finisher, CH_NOID);
    }

    private static final class MinMaxHolder<T> {
        private final Comparator<? super T> comparator;
        private T min;
        private T max;
        private boolean present;

        MinMaxHolder(final Comparator<? super T> comparator) {
        ...
        MinMaxHolder<T> combine(final MinMaxHolder<T> other) {
            if (!other.present) {
                return this;
            } else if (!present) {
                min = other.min;
            ...
            // Keep the left (earlier) value on ties to preserve encounter-order stability.
     ...
     *
     * <p>Floating-point addition remains order-sensitive even with compensated summation,
     * so the returned collector does not have the {@link Characteristics#UNORDERED UNORDERED}
     * characteristic.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T> Collector<T, ?, Double> summingDouble(final ToDoubleFunction<? super T> mapper) throws IllegalArgumentException {
        ...
        return create(SummingDouble_Supplier, accumulator, SummingDouble_Combiner, SummingDouble_Finisher, CH_NOID);
    }
     ...
     *         The values are summed with {@code long} arithmetic, so the total wraps only beyond roughly
     *         {@code 2^32} maximum-magnitude {@code int} values, and the returned mean is then wrong rather
     ...
     */
    public static <T> Collector<T, ?, Double> averagingInt(final ToIntFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * @see #averagingIntOrElseThrow(ToIntFunction)
     */
    public static <T> Collector<T, ?, OptionalDouble> averagingIntOrEmpty(final ToIntFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link NoSuchElementException} if no elements are present in the stream.</p>
     *
     * @param <T> the type of input elements
     * @param mapper a function extracting an integer value from each input element
     * @return a {@code Collector} that produces the arithmetic mean of the extracted integer values as a {@code Double}
     ...
     * @see #averagingIntOrEmpty(ToIntFunction)
     */
    public static <T> Collector<T, ?, Double> averagingIntOrElseThrow(final ToIntFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     *         The values are summed with {@code long} arithmetic, so a total beyond the {@code long} range wraps and
     *         the returned mean is then wrong rather than merely imprecise (as in {@link #summingLong(ToLongFunction)})
     * @throws IllegalArgumentException if {@code mapper} is {@code null}.
     ...
     */
    public static <T> Collector<T, ?, Double> averagingLong(final ToLongFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * @see #averagingLongOrElseThrow(ToLongFunction)
     */
    public static <T> Collector<T, ?, OptionalDouble> averagingLongOrEmpty(final ToLongFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * @see #averagingLongOrEmpty(ToLongFunction)
     */
    public static <T> Collector<T, ?, Double> averagingLongOrElseThrow(final ToLongFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     */
    public static <T> Collector<T, ?, Double> averagingDouble(final ToDoubleFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * @see #averagingDoubleOrElseThrow(ToDoubleFunction)
     */
    public static <T> Collector<T, ?, OptionalDouble> averagingDoubleOrEmpty(final ToDoubleFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);

        final BiConsumer<KahanSummation, T> accumulator = (a, t) -> a.add(mapper.applyAsDouble(t));

        return create(AveragingDouble_Supplier, accumulator, AveragingDouble_Combiner, AveragingDouble_Finisher_op, CH_NOID);
    }
     ...
     * @see #averagingDoubleOrEmpty(ToDoubleFunction)
     */
    public static <T> Collector<T, ?, Double> averagingDoubleOrElseThrow(final ToDoubleFunction<? super T> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);

        final BiConsumer<KahanSummation, T> accumulator = (a, t) -> a.add(mapper.applyAsDouble(t));

        return create(AveragingDouble_Supplier, accumulator, AveragingDouble_Combiner, AveragingDouble_Finisher_orElseThrow, CH_NOID);
    }
     ...
     *
     * <p>The element count is stored in a {@code long} and can wrap on overflow. A wrapped
     ...
     */
    public static <T> Collector<T, ?, BigDecimal> averagingBigInteger(final Function<? super T, BigInteger> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * of elements.</p>
     *
     * <p>The average is calculated as a {@code BigDecimal} with
     * {@link MathContext#DECIMAL128} precision.</p>
     *
     * <p>The element count is stored in a {@code long} and can wrap on overflow. A wrapped
     * nonzero count is used as the divisor; zero is treated like an empty input.</p>
     *
     ...
     * @see #averagingBigIntegerOrEmpty(Function)
     */
    public static <T> Collector<T, ?, BigDecimal> averagingBigIntegerOrElseThrow(final Function<? super T, BigInteger> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * using the provided mapper function, then calculating the arithmetic mean of all values.
     * The result is returned as a {@code BigDecimal}; division is performed with
     * {@link MathContext#DECIMAL128} precision. If no elements
     ...
     *
     * // Average prices with DECIMAL128 precision
     ...
     */
    public static <T> Collector<T, ?, BigDecimal> averagingBigDecimal(final Function<? super T, BigDecimal> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * @see #averagingBigDecimalOrEmpty(Function)
     */
    public static <T> Collector<T, ?, BigDecimal> averagingBigDecimalOrElseThrow(final Function<? super T, BigDecimal> mapper) throws IllegalArgumentException {
        N.checkArgNotNull(mapper, cs.mapper);
     ...
     * in a single, efficient pass through the stream.</p>
     *
     * <p>The collector supports parallel streams, but its floating-point sum is
     * order-sensitive. It therefore retains encounter-order requirements and does not have
     * the {@link Characteristics#UNORDERED UNORDERED} characteristic.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * // returns 24
     *
     * // Concatenate with the empty-string identity, then add a prefix once.
     * // Note the identity really must be an identity: seeding with "start:" instead would make
     * // op.apply(identity, x) != x, and a parallel reduction would then repeat the seed per partition.
     * String result = Stream.of("a", "b", "c")
     *     .collect(Collectors.collectingAndThen(Collectors.reducing("", String::concat), s -> "start:" + s));
     * // returns "start:abc"
     *
     * // Find maximum with default value
    ...
    public static <T> Collector<T, ?, T> reducing(final T identity, final BinaryOperator<T> op) throws IllegalArgumentException {
        ...
        return create(holderSupplier(identity), accumulator, combiner, finisher, CH_NOID);
    }
     ...
     *     .collect(Collectors.<String, String> reducing(
     *         Fn.identity(),
     ...
     * }</pre>
     *
     * <p><b>Note:</b> this overload needs the explicit type witness
     * {@code Collectors.<T, R>reducing(...)} shown above. Without it the compiler cannot separate this
     * method from {@link #reducing(Object, BinaryOperator)} &mdash; the {@code identity} parameter of
     * that overload can itself infer to a functional interface type &mdash; and reports
     * {@code reference to reducing is ambiguous}. If you do not need a mapper, prefer
     * {@link #reducing(BinaryOperator)}, which takes no witness.
     *
     * @param <T> the type of input elements
     * @param <R> the type of the result
     * @param mapper a function to map input elements to the type used for reduction
    ...
    private static <T extends Comparable<? super T>, R> Collector<T, ?, R> naturalMinMax(final Function<NaturalMinMaxHolder<T>, R> finisher) {
        final Supplier<NaturalMinMaxHolder<T>> supplier = NaturalMinMaxHolder::new;
    ...
    private static final class NaturalMinMaxHolder<T extends Comparable<? super T>> implements Consumer<T> {
        ...
        NaturalMinMaxHolder<T> combine(final NaturalMinMaxHolder<T> other) {
            if (!other.present) {
                return this;
            }
            ...
            if (other.hasNonNull) {
                acceptNonNull(other.min);
                acceptNonNull(other.max);
            }

            return this;
        }

        private void acceptNonNull(final T value) {
            if (!hasNonNull) {
                min = value;
                max = value;
                hasNonNull = true;
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link NoSuchElementException} if no elements are present.</p>
     *
     * @param <T> the type of input elements
     * @param <R> the type of the result
     * @param mapper a function to map input elements to the type used for reduction
    ...
    public static Collector<CharSequence, ?, String> commonSuffix() {
        ...
        final BinaryOperator<Pair<CharSequence, Integer>> combiner = (a, b) -> {
            ...
            // b.left() may be null when b's first accumulated element was null (right == 0 then),
            // so only dereference it in the non-zero branch (mirrors the commonPrefix combiner).
            accumulator.accept(a, b.right() == 0 ? "" : b.left().subSequence(b.left().length() - b.right(), b.left().length()));
    ...
    public static <T, K, A, D, M extends Map<K, D>> Collector<T, ?, M> groupingBy(final Function<? super T, ? extends K> keyMapper,
        ...
        return java.util.stream.Collectors.groupingBy(rejectNullKey(keyMapper), mapSupplier, downstream);
    }
    ...
    public static <T, K, A, D, M extends ConcurrentMap<K, D>> Collector<T, ?, M> groupingByConcurrent(final Function<? super T, ? extends K> keyMapper,
        ...
        return java.util.stream.Collectors.groupingByConcurrent(rejectNullKey(keyMapper), mapSupplier, downstream);
    }
     ...
     *
     * <p>The lists preserve stream encounter order. Consequently, the returned collector
     * does not have the {@link Characteristics#UNORDERED UNORDERED} characteristic.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     *
     * <p>The returned collector is unordered only when the downstream collector is unordered;
     * otherwise it preserves the downstream collector's encounter-order requirements.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static <T, A, D> Collector<T, ?, Map<Boolean, D>> partitioningBy(final Predicate<? super T> predicate, final Collector<? super T, A, D> downstream)
        ...
        final Supplier<Map<Boolean, A>> supplier = () -> {
            ...
            map.put(true, downstreamSupplier.get());
            map.put(false, downstreamSupplier.get());
            return map;
        };
        ...
        final Function<Map<Boolean, A>, Map<Boolean, D>> finisher = a -> {
            ...
            result.put(Boolean.TRUE, downstreamFinisher.apply(a.get(Boolean.TRUE)));
            result.put(Boolean.FALSE, downstreamFinisher.apply(a.get(Boolean.FALSE)));
     ...
     *
     * <p>The returned collector is not concurrent. The iteration order of the result is
     * determined by the map created by {@code mapFactory}.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     *
     * <p><b>Null Handling:</b> Both {@code null} keys and {@code null} values are supported if the
     ...
     *
     * <p><b>Null Handling:</b> Both {@code null} keys and {@code null} values are supported if the
     * underlying map implementation allows them. The merge function must handle {@code null} values
     * if they may be present; a {@code null} result from the merge function removes the key.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams.</p>
     *
     * <p><b>Usage Examples:</b></p>
     ...
     */
    public static <K, V> Collector<Map.Entry<K, V>, ?, Map<K, V>> toMap(final BinaryOperator<V> mergeFunction) throws IllegalArgumentException {
     ...
     *     .collect(Collectors.toMap(Suppliers.ofTreeMap()));
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link IllegalStateException} if duplicate keys are encountered.</p>
     *
     ...
     */
    public static <K, V, M extends Map<K, V>> Collector<Map.Entry<K, V>, ?, M> toMap(final Supplier<? extends M> mapFactory) throws IllegalArgumentException {
        N.checkArgNotNull(mapFactory, cs.mapFactory);
     ...
     *     .collect(Collectors.toMap(Integer::sum, () -> new LinkedHashMap<String, Integer>()));
     * }</pre>
     *
     * @param <K> the type of keys
     * @param <V> the type of values
     ...
     * handle {@code null} input elements appropriately.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams.</p>
     *
     * <p><b>Usage Examples:</b></p>
     ...
     * as the merge function must handle {@code null} values if they may be present; a {@code null} result from the
     * merge function removes the key.</p>
     *
     * <p><b>Parallel Stream Support:</b> This collector supports parallel streams.</p>
     *
     * <p><b>Usage Examples:</b></p>
    ...
    public static <T, K, V> Collector<T, ?, Map<K, V>> toLinkedHashMap(final Function<? super T, ? extends K> keyMapper,
        ...
        // Build the collector inline so the LinkedHashMap supplier stays explicit while keeping
        // the ordered CH_ID characteristics used by toMap(...).
     ...
     *
     * <p>As with {@code ConcurrentMap.merge}, if the merge function returns {@code null} for a
     * duplicate key, that key is removed from the result.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * as that of its keys. If duplicate keys are encountered, an {@code IllegalStateException}
     * is thrown; if a duplicate value is encountered, an {@code IllegalArgumentException} is thrown.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link IllegalStateException} if duplicate keys are encountered.
     * During reduction, the returned collector throws {@link IllegalArgumentException} if inserting a new mapping would use a null key or value, or a value for a new key or a merged value is already bound to a different key.</p>
     *
     * @param <T> the type of input elements
     * @param <K> the output type of the key mapping function
     * @param <V> the output type of the value mapping function
     ...
     * If duplicate keys are encountered, an {@code IllegalStateException} is thrown;
     * if a duplicate value is encountered, an {@code IllegalArgumentException} is thrown.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * {@code BiMap::new}. A constructor reference is <i>inexact</i>, so the compiler cannot choose
     * between this overload and {@link #toBiMap(Function, Function, BinaryOperator)} and reports
     * {@code reference to toBiMap is ambiguous}.
     *
     * <p>During reduction, the returned collector throws {@link IllegalStateException} if duplicate keys are encountered.
     * During reduction, the returned collector throws {@link IllegalArgumentException} if inserting a new mapping would use a null key or value, or a value for a new key or a merged value is already bound to a different key.</p>
     *
     ...
     *
     * <p>When duplicate keys are encountered, the provided merge function is used to
     * combine the values. Every value in a {@code BiMap} must be unique: a value produced for a new key, or a
     * merged value, that is already bound to a <i>different</i> key raises an {@code IllegalArgumentException}
     * instead of silently re-binding that value (the {@code forcePut} behaviour of earlier versions). A merged value
     * equal to the one already held by the same key is accepted, and a {@code null} merge result removes the key.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     ...
     * }</pre>
     *
     * <p>During reduction, the returned collector throws {@link IllegalArgumentException} if inserting a new mapping would use a null key or value, or a value for a new key or a merged value is already bound to a different key.</p>
     *
     * @param <T> the type of input elements
     * @param <K> the output type of the key mapping function
     * @param <V> the output type of the value mapping function
    ...
    /**
     * Associates {@code value} with {@code key} in {@code biMap}, merging with any value already
     * mapped to that key, with the {@link Map#merge(Object, Object, BiFunction)} contract: a {@code null}
     * merge result removes the key. A merged value that is already bound to a <i>different</i> key raises the
     * {@code IllegalArgumentException} of {@link BiMap#put(Object, Object)}; earlier versions used
     * {@code forcePut} here and silently stole the value from the other key.
     *
     * @param <K> the key type of the {@code BiMap}
     * @param <V> the value type of the {@code BiMap}
     * @param biMap the {@code BiMap} to update
    ...
    static <K, V> void merge(final BiMap<K, V> biMap, final K key, final V value, final BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
        ...
        if (oldValue == null && !biMap.containsKey(key)) {
            ...
            if (newValue == null) {
                ...
                biMap.put(key, newValue); // put, not forcePut: same key + same value is a no-op, another key's value is an error.
            }
        }
    }

    /**
     * Returns the combiner used by the {@code toBiMap} collectors: it copies every entry of the second
     * {@code BiMap} into the first, resolving key collisions with {@code mergeFunction}, and returns the
     * first map.
     *
     * @param <K> the key type of the {@code BiMap}
     * @param <V> the value type of the {@code BiMap}
     * @param mergeFunction used to resolve collisions between values associated with the same key
     * @return a combiner that merges the second {@code BiMap} into the first
     */
     ...
     * ListMultimap<String, Integer> result = entries.stream()
     *     .collect(Collectors.toMultimap(() -> N.<String, Integer> newListMultimap()));
     ...
     *     .collect(Collectors.toMultimap(
     *         String::length,
     *         () -> N.<Integer, String> newListMultimap()));
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * // Create a multimap with sorted keys
     * ListMultimap<String, Integer> result = items.stream()
     *     .collect(Collectors.toMultimap(
     *         Item::getCategory,
     *         Item::getValue,
     *         Suppliers.ofListMultimap(TreeMap.class)));
     * // Keys are sorted
     * }</pre>
     *
     * @param <T> the type of input elements
    ...
    public static <T, K, V, C extends Collection<V>, M extends Multimap<K, V, C>> Collector<T, ?, M> toMultimap(
        ...
        // Values per key are documented to be collected in encounter order (ListMultimap by default),
        // so this collector must NOT be UNORDERED.
        return create(mapFactory, accumulator, combiner, CH_ID);
    }
     ...
     *         Person::getDepartment,
     *         person -> Stream.of(person.getSkills())));
    ...
    public static <T, K, V> Collector<T, ?, ListMultimap<K, V>> flatMappingValueToMultimap(final Function<? super T, ? extends K> keyMapper,
            final Function<? super T, ? extends Stream<? extends V>> flatValueExtractor) throws IllegalArgumentException {
        N.checkArgNotNull(keyMapper, cs.keyMapper);
        N.checkArgNotNull(flatValueExtractor, cs.flatValueExtractor);
    ...
    public static <T, K, V, C extends Collection<V>, M extends Multimap<K, V, C>> Collector<T, ?, M> flatMappingValueToMultimap(
            final Function<? super T, ? extends K> keyMapper, final Function<? super T, ? extends Stream<? extends V>> flatValueExtractor,
            final Supplier<? extends M> mapFactory) throws IllegalArgumentException {
        N.checkArgNotNull(keyMapper, cs.keyMapper);
        N.checkArgNotNull(flatValueExtractor, cs.flatValueExtractor);
        N.checkArgNotNull(mapFactory, cs.mapFactory);
        ...
        // Per-key values follow encounter order; keep ordered (no UNORDERED); see toMultimap.
        return create(mapFactory, accumulator, combiner, CH_ID);
    }
     ...
     *
     * <p>This collector is similar to {@link #flatMappingValueToMultimap(Function, Function)} but works with
    ...
    public static <T, K, V> Collector<T, ?, ListMultimap<K, V>> flatmappingValueToMultimap(final Function<? super T, ? extends K> keyMapper, // NOSONAR
            final Function<? super T, ? extends Collection<? extends V>> flatValueExtractor) throws IllegalArgumentException {
        N.checkArgNotNull(keyMapper, cs.keyMapper);
        N.checkArgNotNull(flatValueExtractor, cs.flatValueExtractor);
     ...
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
     * // Create a multimap with sorted keys from flat mapped collections
     * ListMultimap<String, String> result = items.stream()
     *     .collect(Collectors.flatmappingValueToMultimap(
     *         Item::getCategory,
     *         Item::getRelatedItems,
     *         Suppliers.ofListMultimap(TreeMap.class)));
     * // Category keys are sorted; values retain encounter order
     * }</pre>
     *
     * @param <T> the type of input elements
     * @param <K> the output type of the key mapping function
    ...
    public static <T, K, V, C extends Collection<V>, M extends Multimap<K, V, C>> Collector<T, ?, M> flatmappingValueToMultimap( // NOSONAR
            final Function<? super T, ? extends K> keyMapper, final Function<? super T, ? extends Collection<? extends V>> flatValueExtractor,
            final Supplier<? extends M> mapFactory) throws IllegalArgumentException {
        N.checkArgNotNull(keyMapper, cs.keyMapper);
        N.checkArgNotNull(flatValueExtractor, cs.flatValueExtractor);
        N.checkArgNotNull(mapFactory, cs.mapFactory);
    ...
    public static <T, K, V> Collector<T, ?, ListMultimap<K, V>> flatMappingKeyToMultimap(final Function<? super T, Stream<? extends K>> flatKeyExtractor,
            final Function<? super T, ? extends V> valueMapper) throws IllegalArgumentException {
        N.checkArgNotNull(flatKeyExtractor, cs.flatKeyExtractor);
        N.checkArgNotNull(valueMapper, cs.valueMapper);
    ...
    public static <T, K, V, C extends Collection<V>, M extends Multimap<K, V, C>> Collector<T, ?, M> flatMappingKeyToMultimap(
            final Function<? super T, Stream<? extends K>> flatKeyExtractor, final Function<? super T, ? extends V> valueMapper,
            final Supplier<? extends M> mapFactory) throws IllegalArgumentException {
        N.checkArgNotNull(flatKeyExtractor, cs.flatKeyExtractor);
        N.checkArgNotNull(valueMapper, cs.valueMapper);
        N.checkArgNotNull(mapFactory, cs.mapFactory);
    ...
    public static <T, K, V> Collector<T, ?, ListMultimap<K, V>> flatmappingKeyToMultimap( // NOSONAR
            final Function<? super T, ? extends Collection<? extends K>> flatKeyExtractor, final Function<? super T, ? extends V> valueMapper)
            throws IllegalArgumentException {
        N.checkArgNotNull(flatKeyExtractor, cs.flatKeyExtractor);
        N.checkArgNotNull(valueMapper, cs.valueMapper);
    ...
    /**
     * Replaces each value of {@code map} in place with the result of applying {@code function} to that
     * entry's key and value, using the map's replacement operation so maps with immutable entry views
     * are supported. Exceptions from {@code function} are propagated unchanged.
     *
     * @param <K> the key type of the map
     * @param <V> the value type of the map
     * @param map the map whose values are replaced
     * @param function applied to each key and value to produce the replacement value
     * @throws ConcurrentModificationException if the backing map detects concurrent structural modification while replacing its entries
     */
    static <K, V> void replaceAll(final Map<K, V> map, final BiFunction<? super K, ? super V, ? extends V> function) throws ConcurrentModificationException {
     ...
     * @throws IllegalArgumentException if any of {@code downstream1}, {@code downstream2}, or {@code merger} is
     *         {@code null}.
     */
     ...
     * </ul>
     *
     * <p>All mapper arguments are required to be non-null and are validated when the
     * collector is created, rather than on the first accumulated element.</p>
     *
     * <p><b>Usage Examples:</b></p>
     * <pre>{@code
    ...
    public static final class MoreCollectors extends Collectors {
         ...
         *
         * <p>The collector handles integer overflow by wrapping around as specified by
         * Java's integer arithmetic rules. The tuple of sums is independent of encounter
         * order.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
        ...
        public static <T> Collector<T, ?, Tuple2<Integer, Integer>> summingInt(final ToIntFunction<? super T> firstMapper,
            ...
            N.checkArgNotNull(firstMapper, cs.firstMapper);
        ...
        public static <T> Collector<T, ?, Tuple3<Integer, Integer, Integer>> summingInt(final ToIntFunction<? super T> firstMapper,
            ...
            N.checkArgNotNull(firstMapper, cs.firstMapper);
            N.checkArgNotNull(secondMapper, cs.secondMapper);
            N.checkArgNotNull(thirdMapper, cs.thirdMapper);
         ...
         * but returns the sums as {@code long} values so totals may exceed the {@code int} range.
         * This is particularly useful when summing values that might exceed the range
         * of {@code int}.</p>
         *
         * <p>Each integer value is widened to {@code long} before addition, and the result is
         * independent of encounter order. The {@code long} sums still wrap on overflow.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
         * // Calculate totals that may exceed the int range
         ...
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
         * // Calculate metrics that may exceed the int range
         ...
         * the same stream in a single pass.</p>
         *
         * <p>The collector handles long overflow by wrapping around as specified by
         * Java's long arithmetic rules. The tuple of sums is independent of encounter
         * order.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
         ...
         * the same stream in a single pass.</p>
         *
         * <p>The computation is performed in encounter order. Floating-point addition remains
         * order-sensitive even with compensated summation, so the returned collector does not
         * have the {@link Characteristics#UNORDERED UNORDERED} characteristic. Special
         * floating-point values (NaN and infinity) follow IEEE 754 arithmetic.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
        ...
        public static <T> Collector<T, ?, Tuple2<Double, Double>> summingDouble(final ToDoubleFunction<? super T> firstMapper,
            ...
            return create(SummingDouble_Supplier_2, accumulator, SummingDouble_Combiner_2, SummingDouble_Finisher_2, CH_NOID);
        }
        ...
        public static <T> Collector<T, ?, Tuple3<Double, Double, Double>> summingDouble(final ToDoubleFunction<? super T> firstMapper,
            ...
            return create(SummingDouble_Supplier_3, accumulator, SummingDouble_Combiner_3, SummingDouble_Finisher_3, CH_NOID);
        }
         ...
         *
         * <p>The result is independent of encounter order. {@code BigInteger} arithmetic is
         * exact and does not overflow.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
         ...
         *
         * <p>The result is independent of encounter order. {@code BigDecimal} addition is exact
         * and preserves all decimal places.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
         ...
         * Floating-point addition remains order-sensitive even with compensated summation,
         * so the returned collector does not have the
         * {@link Characteristics#UNORDERED UNORDERED} characteristic.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
        ...
        public static <T> Collector<T, ?, Tuple2<Double, Double>> averagingDouble(final ToDoubleFunction<? super T> firstMapper,
            ...
            return create(AveragingDouble_Supplier_2, accumulator, AveragingDouble_Combiner_2, AveragingDouble_Finisher_2, CH_NOID);
        }
        ...
        public static <T> Collector<T, ?, Tuple3<Double, Double, Double>> averagingDouble(final ToDoubleFunction<? super T> firstMapper,
            ...
            return create(AveragingDouble_Supplier_3, accumulator, AveragingDouble_Combiner_3, AveragingDouble_Finisher_3, CH_NOID);
        }
         ...
         * average as {@code BigDecimal} values with {@link MathContext#DECIMAL128} precision.</p>
         *
         ...
         * {@code BigDecimal} values with {@link MathContext#DECIMAL128} precision.</p>
         *
         ...
         * without binary floating-point conversion. The collector accumulates the sum of values from each
         * mapper and counts the elements, then computes the averages with
         * {@link MathContext#DECIMAL128} precision.</p>
         *
         * <p>The returned collector produces a {@code Tuple2} containing the averages of
         * the values extracted by each mapper function. Each average is calculated as
         ...
         *
         * <p>This collector is useful when you need to compute averages of decimal values
         * for multiple properties simultaneously. The collector accumulates the sum of values
         * from each mapper and counts the elements, then computes the averages with
         * {@link MathContext#DECIMAL128} precision.</p>
         *
         * <p>The returned collector produces a {@code Tuple3} containing the averages of
         * the values extracted by each mapper function. Each average is calculated as
        ...
        public static <T, R1, R2, R3, R4, R5> Collector<T, ?, Tuple5<R1, R2, R3, R4, R5>> combine(final Collector<? super T, ?, R1> downstream1,
            ...
            N.checkArgNotNull(downstream1, cs.downstream1); //NOSONAR
            N.checkArgNotNull(downstream2, cs.downstream2); //NOSONAR
            N.checkArgNotNull(downstream3, cs.downstream3); //NOSONAR
            N.checkArgNotNull(downstream4, cs.downstream4); //NOSONAR
            N.checkArgNotNull(downstream5, cs.downstream5); //NOSONAR
         ...
         * excluding {@code IDENTITY_FINISH}. The component sequence is captured when this
         * method is called; later structural changes to {@code downstreams} are not reflected.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
        ...
        public static <T, R> Collector<T, ?, R> combine(final Collection<? extends Collector<? super T, ?, ?>> downstreams, final Function<Object[], R> merger)
            ...
            final List<Collector<? super T, ?, ?>> downstreamList = new ArrayList<>(downstreams.size());
            int downstreamIndex = 0;
            for (final Collector<? super T, ?, ?> downstream : downstreams) {
                N.checkArgNotNull(downstream, "downstreams[" + downstreamIndex++ + "]");
            ...
            final BiConsumer<Object, ? super T>[] accumulators = downstreamList.stream().map(Collector::accumulator).toArray(i -> new BiConsumer[size]);
            final BinaryOperator<Object>[] combiners = downstreamList.stream().map(Collector::combiner).toArray(i -> new BinaryOperator[size]);
            final Function<Object, Object>[] finishers = downstreamList.stream().map(Collector::finisher).toArray(i -> new Function[size]);
            ...
            final Collection<Characteristics> common = N.intersection(downstreamList.stream().map(Collector::characteristics).toList());
        ...
        public static <T> Collector<T, ?, Dataset> toDataset() {
            ...
            final Function<List<T>, Dataset> finisher = it -> N.newDataset(it);
         ...
         *
         * <p>The returned collector accumulates elements into a list and then creates a
         * {@code Dataset} from that list using the specified column names. Column names are required:
         * a {@code null} or empty {@code columnNames} is rejected with {@link IllegalArgumentException},
         * consistent with {@link com.landawn.abacus.util.stream.Stream#toDataset(java.util.List) Stream.toDataset}
         * and {@link com.landawn.abacus.util.N#newDataset(java.util.Collection, java.util.Collection) N.newDataset}.</p>
         *
         * <p><b>Usage Examples:</b></p>
         * <pre>{@code
