...
final class ParallelIteratorStream<T> extends IteratorStream<T> {
     ...
     * Elements are pulled from the underlying iterator and tested under the same lock until the
     * first non-matching element is found. That element and all remaining elements are retained.
     *
    ...
    public <E extends Exception> void forEachPair(final int increment, final Throwables.BiConsumer<? super T, ? super T, E> action)
        ...
        for (int i = 0; i < maxThreadNum; i++) {
            asyncExecutorToUse = execute(asyncExecutorToUse, maxThreadNum, i, futureList, eHolder, new Runnable() {
    ...
    public Stream<T> sequential() throws IllegalStateException {
        ...
        if (tmp == null) {
            synchronized (this) {
                tmp = sequential;
                if (tmp == null) {
                    tmp = new IteratorStream<>(elements, isSorted(), comparator(), closeHandlersForNewStream());
                    sequential = tmp;
                }
            }
        }
