...
final class ParallelArrayCharStream extends ArrayCharStream {
     ...
     *
     * @param values the char array to stream; it is used directly, not copied, so callers must not mutate it afterwards
    ...
    public CharStream sequential() throws IllegalStateException {
        ...
        if (tmp == null) {
            synchronized (this) {
                tmp = sequential;
                if (tmp == null) {
                    tmp = new ArrayCharStream(elements, fromIndex, toIndex, isSorted(), closeHandlersForNewStream());
                    sequential = tmp;
                }
            }
        }
