 ...
 *
 * <p>ConcurrentCacheMap is designed as a lightweight caching map for scenarios where:</p>
 * <ul>
 ...
 * <pre>{@code
 * // Create a cache for values by name
 * ConcurrentCacheMap<String, Integer> cache = new ConcurrentCacheMap<>(128);
 *
 * // Cache a value
 * cache.put("answer", 42);
 *
...
public final class ConcurrentCacheMap<K, V> extends AbstractMap<K, V> {
    ...
    /**
     * Constructs a ConcurrentCacheMap with the specified initial capacity.
     ...
     * <pre>{@code
     * ConcurrentCacheMap<String, Integer> cache = new ConcurrentCacheMap<>(128);
     * boolean empty = cache.isEmpty();   // returns true
     * cache.put("a", 1);
     * int size = cache.size();           // returns 1
     * }</pre>
     *
     ...
     * cache.put("answer", 42);
     * Integer value = cache.get("answer");
     * Object result = cache.get(null);   // returns null instead of throwing
     * }</pre>
     *
     ...
     * ConcurrentCacheMap<String, Integer> cache = new ConcurrentCacheMap<>(16);
     * cache.put("answer", 42);
     * Integer oldValue = cache.put("answer", 43);  // returns 42
     * }</pre>
     *
     ...
     * <pre>{@code
     * ConcurrentCacheMap<String, Integer> cache = new ConcurrentCacheMap<>(16);
     * Integer prior = cache.putIfAbsent("answer", 42);
     * if (prior != null) {
     ...
     * ConcurrentCacheMap<String, Integer> cache = new ConcurrentCacheMap<>(16);
     * cache.put("answer", 42);
     * Integer removed = cache.remove("answer");
     * cache.remove(null);   // returns null instead of throwing
     * }</pre>
     *
     ...
     *     Integer value = cache.get("answer");
     * }
     * cache.containsKey(null);   // returns false instead of throwing
     * }</pre>
     *
     ...
     * ConcurrentCacheMap<String, Integer> cache = new ConcurrentCacheMap<>(16);
     * cache.put("answer", 42);
     * if (cache.containsValue(42)) {
     *     // The value has been cached
     * }
     * cache.containsValue(null);   // returns false instead of throwing
     * }</pre>
     *
     ...
     *
     * <p>If {@code other} is also a {@code ConcurrentCacheMap}, the comparison is performed
    ...
    public boolean equals(Object other) {
        ...
        if (other instanceof ConcurrentCacheMap op) {
