...
public class ImmutableEntryTest extends TestBase {
    ...
    @Test
    public void testCopyOf_FromImmutableEntry() {
        ImmutableEntry<String, Integer> original = ImmutableEntry.of("key", 42);
        ImmutableEntry<String, Integer> copy = ImmutableEntry.copyOf(original);

        Assertions.assertEquals(original.getKey(), copy.getKey());
        Assertions.assertEquals(original.getValue(), copy.getValue());
    ...
    @Test
    public void testSetValue_AfterCopyOf_ThrowsUnsupported() {
        Map.Entry<String, Integer> mutableEntry = new AbstractMap.SimpleEntry<>("key", 1);
        ImmutableEntry<String, Integer> immutable = ImmutableEntry.copyOf(mutableEntry);

        Assertions.assertThrows(UnsupportedOperationException.class, () -> immutable.setValue(2));
        Assertions.assertEquals(1, immutable.getValue());
    }
}
