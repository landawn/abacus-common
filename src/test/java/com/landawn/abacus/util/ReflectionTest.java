...
public class ReflectionTest extends TestBase {
    ...
    // TODO: The non-ReflectASM code paths in get(), set(), invoke(), call(), getField(), getDeclaredMethod()
    // are only exercised when ReflectASM is NOT on the classpath. Since ReflectASM is available in this
    // environment, these paths cannot be easily tested without modifying source code.

    @Test
    public void testDirectConstructorWithInstance() {
        // Exercise Reflection(Class, Object) constructor directly (same package access)
        TestClass instance = new TestClass("direct");
        Reflection<TestClass> ref = new Reflection<>(TestClass.class, instance);
    ...
    @Test
    public void testNewInstanceCaching_SameClass() {
        // Call newInstance multiple times to exercise constructor caching
        Reflection<TestClass> ref = Reflection.on(TestClass.class);
        Reflection<TestClass> r1 = ref.newInstance("first");
        Reflection<TestClass> r2 = ref.newInstance("second");
        Assertions.assertEquals("first", r1.instance().publicField);
        Assertions.assertEquals("second", r2.instance().publicField);
    }
    ...
    @Test
    public void testGet_PrivateFieldFromSuperclass() {
        String value = Reflection.on(new SubTestClass()).get("privateField");
        Assertions.assertEquals("privateValue", value);
    }

    @Test
    public void testGetAndSetCaching_MultipleFields() {
        TestClass instance = new TestClass();
        Reflection<TestClass> ref = Reflection.on(instance);
        // Access fields multiple times to trigger caching
        ref.set("publicField", "v1");
        ref.set("publicField", "v2");
        String val = ref.get("publicField");
        Assertions.assertEquals("v2", val);
    }
    ...
    @Test
    public void testSet_PrivateFieldFromSuperclass() {
        SubTestClass target = new SubTestClass();
        Reflection.on(target).set("privateField", "updated");
        Assertions.assertEquals("updated", Reflection.on(target).get("privateField"));
    }
    ...
    @Test
    public void testInvokeSameMethodMultipleTimes() {
        // Exercises method caching in getDeclaredMethod
        TestClass instance = new TestClass();
        Reflection<TestClass> ref = Reflection.on(instance);
        for (int i = 0; i < 3; i++) {
            Integer result = ref.invoke("calculate", i, 1);
            Assertions.assertEquals(i + 1, (int) result);
        }
    }
    ...
    @Test
    public void testCallSameMethodMultipleTimes() {
        TestClass instance = new TestClass();
        Reflection<TestClass> ref = Reflection.on(instance);
        ref.call("setPublicField", "a");
        ref.call("setPublicField", "b");
        ref.call("setPublicField", "c");
        Assertions.assertEquals("c", instance.publicField);
    }
    ...
    @Test
    public void testGetTypes_WithNullArg() {
        // invoke with a null arg exercises getTypes(null)
        TestClass instance = new TestClass();
        Reflection<TestClass> ref = Reflection.on(instance);
