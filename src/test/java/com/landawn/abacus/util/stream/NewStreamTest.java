...
public class NewStreamTest {
    ...
    public void test_cartesianProduct() {
        assertDoesNotThrow(() -> {
            Stream.<Object> of("a", "b", "c").cartesianProduct(N.toList("1", 2, 3), N.toList('x', 'y', 'z')).limit(5).forEach(Fn.println());
            N.println("----------------------");
            Stream.<Object> of("a", "b", "c").cartesianProduct(N.toList("1", 2, 3), N.toList('x', 'y', 'z')).skip(3).limit(5).forEach(Fn.println());
        });
    }
